...
import { createInsuranceSchema } from '@/modules/vehicles/vehicles.schema'
...
export async function GET(_req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const id = vehicle.id
    const items = await insuranceService.getMany(id, session.userId)
    ...
    console.error('[GET /api/vehicles/[slug]/insurance]', err)
...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const validated = createInsuranceSchema.safeParse(body)
      if (!validated.success) return apiError('VALIDATION_ERROR', 400, validated.error)

      const insurance = await insuranceService.create(validated.data, id, session.userId)
    ...
    console.error('[POST /api/vehicles/[slug]/insurance]', err)
    return apiError('INTERNAL_ERROR', 500)
  }
}
