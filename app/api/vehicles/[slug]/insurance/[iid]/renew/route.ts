...
import { renewInsuranceSchema } from '@/modules/vehicles/vehicles.schema'
...
export async function POST(
  ...
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const validated = renewInsuranceSchema.safeParse(body)
      if (!validated.success) return apiError('VALIDATION_ERROR', 400, validated.error)
      const insurance = await insuranceService.renew(iid, id, validated.data, session.userId)
    ...
    console.error('[POST /api/vehicles/[slug]/insurance/[iid]/renew]', err)
