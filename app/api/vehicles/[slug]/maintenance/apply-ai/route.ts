...
import { applyAiSuggestionsSchema } from '@/modules/vehicles/vehicles.schema'
...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const validated = applyAiSuggestionsSchema.safeParse(body)
      if (!validated.success) return apiError('VALIDATION_ERROR', 400, validated.error)

      const { visitId, ...rest } = body as { visitId?: string }
      if (!visitId || typeof visitId !== 'string') {
        return apiError('VALIDATION_ERROR', 400)
      }

      const items = await maintenanceService.applyAiSuggestions(
        visitId,
        id,
    ...
    console.error('[POST /api/vehicles/[slug]/maintenance/apply-ai]', err)
