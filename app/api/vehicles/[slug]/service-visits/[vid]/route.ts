...
import { updateServiceVisitSchema } from '@/modules/vehicles/vehicles.schema'
...
export async function PATCH(
  ...
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const validated = updateServiceVisitSchema.safeParse(body)
      if (!validated.success) return apiError('VALIDATION_ERROR', 400, validated.error)

      const visit = await serviceVisitService.update(vid, id, validated.data, session.userId)
    ...
    console.error('[PATCH /api/vehicles/[slug]/service-visits/[vid]]', err)
...
export async function DELETE(
  ...
  try {
    ...
    const id = vehicle.id
    return withRateLimit(session.userId, async () => {
      await serviceVisitService.delete(vid, id, session.userId)
    ...
    console.error('[DELETE /api/vehicles/[slug]/service-visits/[vid]]', err)
    return apiError('INTERNAL_ERROR', 500)
  }
}
