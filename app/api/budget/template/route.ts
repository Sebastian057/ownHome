...
import { updateTemplateCurrencySchema } from '@/modules/budget/budget.schema'
...
export async function GET() {
  try {
    ...
    console.error('[GET /api/budget/template]', err)
...
export async function PUT(req: Request) {
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const validated = updateTemplateCurrencySchema.safeParse(body)
      if (!validated.success) return apiError('VALIDATION_ERROR', 400, validated.error.flatten())

      await budgetService.updateTemplateCurrency(session.userId, validated.data.currency)
      const template = await budgetService.getTemplate(session.userId)
      return apiSuccess(template)
    })
  } catch (err) {
    if (err instanceof AppError) return apiError(err.code, err.code === 'UNAUTHORIZED' ? 401 : 500)
    console.error('[PUT /api/budget/template]', err)
    return apiError('INTERNAL_ERROR', 500)
  }
}
