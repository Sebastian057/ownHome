...
import { createIncomeSchema } from '@/modules/budget/budget.schema'
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const incomes = await budgetService.listIncomes(id, session.userId)
    ...
    console.error('[GET /api/budget/periods/[id]/incomes]', err)
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return withRateLimit(session.userId, async () => {
      ...
      const income = await budgetService.createIncome(id, session.userId, validated.data)
    ...
    console.error('[POST /api/budget/periods/[id]/incomes]', err)
    return apiError('INTERNAL_ERROR', 500)
  }
}
